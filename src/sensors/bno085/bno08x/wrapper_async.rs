...
// use super::{
//     activity::Activity,
//     interface::{PACKET_HEADER_LENGTH, SensorInterfaceAsync},
// };
...
//     significant_motion_detected: bool,
//     step_count: u16,
//     step_detected: bool,
//     shake_detected: bool,
//     activity: Activity,
...
//     pub async fn handle_all_messages(&mut self, delay: &mut impl DelayNs, timeout_ms: u8) -> u32 {
//         let mut total_handled: u32 = 0;
//         loop {
//             let handled_count = self.handle_one_message(delay, timeout_ms).await;
//             if handled_count == 0 {
//                 break;
//             } else {
...
//     pub async fn handle_one_message(&mut self, delay: &mut impl DelayNs, max_ms: u8) -> u32 {
//         let mut msg_count = 0;
...
//     pub async fn init(&mut self, delay_source: &mut impl DelayNs) -> Result<(), WrapperError<SE>> {
...
//             self.handle_received_packet(received_len);
//         }
//         // self.send_packet(CHANNEL_EXECUTABLE, &data).await?;
...
//     pub async fn enable_significant_motion(&mut self) -> Result<(), WrapperError<SE>> {
//         self.enable_report(SENSOR_REPORTID_SIGNIFICANT_MOTION, 0)
//             .await
//     }

//     pub async fn enable_step_counter(
//         &mut self,
//         millis_between_reports: u16,
//     ) -> Result<(), WrapperError<SE>> {
//         self.enable_report(SENSOR_REPORTID_STEP_COUNTER, millis_between_reports)
//             .await
//     }
...
//         let val = self.significant_motion_detected;
//         self.significant_motion_detected = false; // clear after reading
//         val
//     }
...
//     pub fn step_detected(&mut self) -> bool {
//         let val = self.step_detected;
//         self.step_detected = false;
//         val
//     }

//     pub fn shake_detected(&mut self) -> bool {
//         let val = self.shake_detected;
...
//     pub async fn enable_activity_classifier(
//         &mut self,
//         millis_between_reports: u16,
//     ) -> Result<(), WrapperError<SE>> {
//         self.enable_report(SENSOR_REPORTID_ACTIVITY_CLASSIFIER, millis_between_reports)
//             .await
//     }
...
//         let val = self.stability_detected;
//         self.stability_detected = false;
//         val
//     }

//     pub fn pickup_detected(&mut self) -> bool {
//         let val = self.pickup_detected;
//         self.pickup_detected = false;
//         val
//     }

//     pub async fn sleep(&mut self) -> Result<(), WrapperError<SE>> {
//         let data: [u8; 1] = [EXECUTABLE_DEVICE_CMD_SLEEP];
//         self.send_packet(CHANNEL_EXECUTABLE, &data).await?;
//         Ok(())
//     }

//     pub async fn wake(&mut self) -> Result<(), WrapperError<SE>> {
//         let data: [u8; 1] = [EXECUTABLE_DEVICE_CMD_ON];
//         self.send_packet(CHANNEL_EXECUTABLE, &data).await?;
//         Ok(())
//     }
...
// const SENSOR_REPORTID_SIGNIFICANT_MOTION: u8 = 0x12;
// const SENSOR_REPORTID_STEP_COUNTER: u8 = 0x11;
// const SENSOR_REPORTID_STEP_DETECTOR: u8 = 0x18;
// const SENSOR_REPORTID_SHAKE_DETECTOR: u8 = 0x19;
// const SENSOR_REPORTID_ACTIVITY_CLASSIFIER: u8 = 0x1E;
// const SENSOR_REPORTID_STABILITY_DETECTOR: u8 = 0x1C;
// const SENSOR_REPORTID_PICKUP_DETECTOR: u8 = 0x1B;
...
impl<SI> BNO085Async<SI> {
    /// Resynchronise the driver's SHTP state with a device that has just reset.
    ///
    /// The device restarts its per-channel sequence numbers at zero on reset, but
    /// `sequence_numbers` here is only zeroed in `new_with_interface` — `init`
    /// does not touch it. So after an executable ON (or a hardware RST), which
    /// both reset the hub, the driver keeps counting from where it left off while
    /// the device expects to start again from zero. At cold boot the two agree,
    /// which is why identical command sequences work at boot and not after a
    /// wake. Call this immediately after any device reset, before `init`.
    pub fn resync_after_device_reset(&mut self) {
        self.sequence_numbers = [0; NUM_CHANNELS];
        self.last_packet_len_received = 0;
        self.device_reset = false;
        self.prod_id_verified = false;
        self.init_received = false;
        self.advert_received = false;
        self.error_list_received = false;
    }
    ...
    SE: core::fmt::Debug,
{
        ...
        match chan_num {
            ...
            CHANNEL_HUB_CONTROL => {
                match report_id {
                    ...
                    SHUB_GET_FEATURE_RESP => {
                        // 0xFC — body: [0xFC, reportID, flags, chgSensLSB,
                        // chgSensMSB, interval bytes 0..3, ...]. Log what the hub
                        // believes the config is, to distinguish "Set Feature was
                        // accepted but no reports are produced" from "Set Feature
                        // never landed".
                        let m = &self.packet_recv_buf[..received_len];
                        if m.len() >= PACKET_HEADER_LENGTH + 9 {
                            let b = &m[PACKET_HEADER_LENGTH..];
                            let interval = (b[5] as u32)
                                | ((b[6] as u32) << 8)
                                | ((b[7] as u32) << 16)
                                | ((b[8] as u32) << 24);
                            defmt::info!(
                                "DIAG GetFeatureResp: id=0x{:02X} flags=0x{:02X} interval_us={}",
                                b[1],
                                b[2],
                                interval
                            );
    ...
    /// Ask the hub to report its current configuration for `report_id`. The
    /// reply (0xFC) is logged by the message handler.
    pub async fn request_feature(&mut self, report_id: u8) -> Result<(), WrapperError<SE>> {
        let cmd_body: [u8; 2] = [SHUB_GET_FEATURE_REQ, report_id];
        self.send_packet(CHANNEL_HUB_CONTROL, &cmd_body).await?;
        Ok(())
    }
    ...
    async fn enable_report(
        ...
        self.enable_report_with_flags(report_id, millis_between_reports, 0)
            .await
    }

    /// Enable rotation vector with the wake + always-on feature flags (0x0C)
    /// set, i.e. the same flags significant motion uses. DIAGNOSTIC: if the hub
    /// is still in its reduced "sleep" power state after the executable ON, only
    /// wake/always-on sensors run — so a plain (flags=0) rotation vector stays
    /// silent while this one reports.
    pub async fn enable_rotation_vector_always_on(
        &mut self,
        millis_between_reports: u16,
    ) -> Result<(), WrapperError<SE>> {
        self.enable_report_with_flags(SENSOR_REPORTID_ROTATION_VECTOR, millis_between_reports, 0x0C)
            .await
    }

    /// Enable a particular report, specifying the SH-2 feature flags byte
    /// (bit 2 = wake-up enable, bit 3 = always-on enable).
    async fn enable_report_with_flags(
        &mut self,
        report_id: u8,
        millis_between_reports: u16,
        feature_flags: u8,
...
/// Unit quaternion rotation vector, Q point 12, with heading accuracy estimate (radians)
const SENSOR_REPORTID_ROTATION_VECTOR: u8 = 0x05;
/// Public alias of the rotation vector report id, for feature queries.
pub const ROTATION_VECTOR_REPORT_ID: u8 = SENSOR_REPORTID_ROTATION_VECTOR;
// const SENSOR_REPORTID_GRAVITY: u8 = 0x06; // Q point 8
/// Gyroscope uncalibrated (rad/s): Q point 9
const SENSOR_REPORTID_GYRO: u8 = 0x07;
// 0x08 game rotation vector : Q point 14
