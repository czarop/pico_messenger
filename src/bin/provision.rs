...
//     defmt::info!("delete response: {:?}", &buf[..n]);
...
//     let n = read_line(&mut uart, &mut buf);
//     defmt::info!("hex mode response: {:?}", &buf[..n]);

//     if &buf[..n] == b"\r\nOK\r\n" {
//         defmt::info!("Hex mode SUCCESS. Saving to NVM...");
...
//         let n = read_line(&mut uart, &mut buf);
//         defmt::info!("binary mode command response: {:?}", &buf[..n]);
...
//             let n = read_line(&mut uart, &mut buf);
//             defmt::info!("binary mode data response: {:?}", &buf[..n]);

//             if &buf[..n] == b"\r\nOK\r\n" {
//                 defmt::info!("Binary mode SUCCESS. Saving to NVM...");
//                 uart.blocking_write(b"AT#RESET=1\r").unwrap();
//                 defmt::info!("Done. Wait 15s then flash main firmware.");
//             } else {
