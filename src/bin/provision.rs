...
#[allow(dead_code)]
const T3412_PRODUCTION: &[u8] = b"00100100"; // 4hr
...
async fn main(_spawner: Spawner) {
    ...
    if stored {
        ...
        if PROVISION_PSM {
            ...
            defmt::info!("Arming RING pin (AT#RINGPIN=0)...");
            uart.blocking_write(b"AT#RINGPIN=0\r").unwrap();
            let n = read_line(&mut uart, &mut buf);
